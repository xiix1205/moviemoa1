package vo;

public class NewsBean {

	private int news_num;
	private String news_name;        
	private String news_subject;      
	private String img_a;     
	private String content_a; 
	private String img_b;     
	private String content_b; 	
	private String img_c;     
	private String content_c; 
	private String img_d;     
	private String content_d; 
	private String img_e;     
	private String content_e; 
	private String img_f;     
	private String content_f; 
	private String news_date;     
	private int count;
	
	public NewsBean() {
		super();
		// TODO Auto-generated constructor stub
	}

	public NewsBean(int news_num, String news_name, String news_subject, String img_a, String content_a, String img_b,
			String content_b, String img_c, String content_c, String img_d, String content_d, String img_e,
			String content_e, String img_f, String content_f, String news_date, int count) {
		super();
		this.news_num = news_num;
		this.news_name = news_name;
		this.news_subject = news_subject;
		this.img_a = img_a;
		this.content_a = content_a;
		this.img_b = img_b;
		this.content_b = content_b;
		this.img_c = img_c;
		this.content_c = content_c;
		this.img_d = img_d;
		this.content_d = content_d;
		this.img_e = img_e;
		this.content_e = content_e;
		this.img_f = img_f;
		this.content_f = content_f;
		this.news_date = news_date;
		this.count = count;
	}

	public int getNews_num() {
		return news_num;
	}

	public void setNews_num(int news_num) {
		this.news_num = news_num;
	}

	public String getNews_name() {
		return news_name;
	}

	public void setNews_name(String news_name) {
		this.news_name = news_name;
	}

	public String getNews_subject() {
		return news_subject;
	}

	public void setNews_subject(String news_subject) {
		this.news_subject = news_subject;
	}

	public String getImg_a() {
		return img_a;
	}

	public void setImg_a(String img_a) {
		this.img_a = img_a;
	}

	public String getContent_a() {
		return content_a;
	}

	public void setContent_a(String content_a) {
		this.content_a = content_a;
	}

	public String getImg_b() {
		return img_b;
	}

	public void setImg_b(String img_b) {
		this.img_b = img_b;
	}

	public String getContent_b() {
		return content_b;
	}

	public void setContent_b(String content_b) {
		this.content_b = content_b;
	}

	public String getImg_c() {
		return img_c;
	}

	public void setImg_c(String img_c) {
		this.img_c = img_c;
	}

	public String getContent_c() {
		return content_c;
	}

	public void setContent_c(String content_c) {
		this.content_c = content_c;
	}

	public String getImg_d() {
		return img_d;
	}

	public void setImg_d(String img_d) {
		this.img_d = img_d;
	}

	public String getContent_d() {
		return content_d;
	}

	public void setContent_d(String content_d) {
		this.content_d = content_d;
	}

	public String getImg_e() {
		return img_e;
	}

	public void setImg_e(String img_e) {
		this.img_e = img_e;
	}

	public String getContent_e() {
		return content_e;
	}

	public void setContent_e(String content_e) {
		this.content_e = content_e;
	}

	public String getImg_f() {
		return img_f;
	}

	public void setImg_f(String img_f) {
		this.img_f = img_f;
	}

	public String getContent_f() {
		return content_f;
	}

	public void setContent_f(String content_f) {
		this.content_f = content_f;
	}

	public String getNews_date() {
		return news_date;
	}

	public void setNews_date(String news_date) {
		this.news_date = news_date;
	}

	public int getCount() {
		return count;
	}

	public void setCount(int count) {
		this.count = count;
	}
	
	
}
